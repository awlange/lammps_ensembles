...
protected:
  ...
  // awgl: added functions for modifying with LAMMPS ensembles
public:
  virtual cvm::real extract_value(int which) { return 0.0; }; 
...
protected:
  ...
  // awgl: added functions for modifying with LAMMPS ensembles
public:
  virtual cvm::real extract_value(int which)
  {
    if      (which == 0) return force_k;
    else if (which == 1) return colvar_centers[0];
    else return -999.9; 
  }; 
};
